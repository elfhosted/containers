...
# Start PD in the background
tmux -f /usr/src/app/restricted.tmux.conf new-session -x 80 -y 24 -A -d -s plex_debrid /plex-debrid.sh

# Prepare a tmux entry to the already-running process, use solarized dark colors
ttyd -p 3001 -W \
 -t titleFixed='plex_debrid | ElfHosted' \
