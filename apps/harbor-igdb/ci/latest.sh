#!/usr/bin/env bash
# harbor-igdb: one of the nine Harbor backend services.
#
# Source: https://github.com/harborstremio/harbor-hosted (PRIVATE), at
# services/harbor-igdb/src. These images integrate with ElfHosted-internal
...
# tag-separator "-", so tags read harbor-igdb-v0.1.0. That form is a valid git ref
# and a valid Docker tag, so it is emitted verbatim.
#
# FALLBACK: no harbor-igdb-v* tag exists yet, because the vendoring PR (#1) is
...
component="harbor-igdb"
branch="feat/vendored-services-and-deployment"
auth_header="Authorization: Bearer ${ZURG_GH_CREDS}"

# Newest harbor-igdb-v* tag. Sorted by version rather than trusting the API's
