...
# private docs repo.
version=$(curl -L -sX GET "https://api.github.com/repos/elfhosted/docs.elfhosted.com/commits/main" --header "Authorization: Bearer ${ZURG_GH_CREDS}" | jq --raw-output '.sha[0:7]')
printf "%s" "${version}"
