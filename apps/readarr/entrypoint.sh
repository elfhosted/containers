...
if [[ "${USE_POSTGRESQL:-"false"}" == "true" ]]; then
    ...
    if [[ -f /config/i-am-bootstrapped && -f /config/readarr.db ]]; then
        echo "Migrating to postgresql database..."
        
        # Create databases
        psql -c "DROP DATABASE IF EXISTS readarr_main;" && psql -c "CREATE DATABASE readarr_main;" && psql -c "ALTER DATABASE readarr_main OWNER TO readarr;"
        psql -c "DROP DATABASE IF EXISTS readarr_logs;" && psql -c "CREATE DATABASE readarr_logs;" && psql -c "ALTER DATABASE readarr_logs OWNER TO readarr;"
  
        # Start readarr to force the database schemas to be created
        ...
        pg_dump --schema-only -d readarr_main > /tmp/readarr_main_schema.sql

        # Recreate database from schemas
        echo "Recreating databases..."
        psql -c "DROP DATABASE IF EXISTS readarr_main;" && psql -c "CREATE DATABASE readarr_main;" && psql -c "ALTER DATABASE readarr_main OWNER TO readarr;" && psql -d readarr_main -f /tmp/readarr_main_schema.sql

        # Import sqlite data
        echo "Importing SQLite databases..."
        POSTGRES_CONN_STRING=postgres://readarr:readarr@localhost/readarr_main?sslmode=disable SQLITE_CONN_STRING=/config/readarr.db importarr

        # Move sqlite files into migrated folder
        echo "Archiving SQLite databases"
        mkdir -p /config/migrated-to-postgres
        mv /config/readarr.db /config/migrated-to-postgres
