...
elif [[ "${channel}" == "compression" ]]; then
    version=$(curl -sX GET "https://api.github.com/repos/elfhosted/nzbdav/commits/feature/sqlite-maintenance" --header "Authorization: Bearer ${TOKEN}" | jq --raw-output '.sha')
else
