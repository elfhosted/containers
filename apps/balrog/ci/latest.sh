...
version=$(curl -L -sX GET https://api.github.com/repos/elfhosted/balrog/releases/latest --header "Authorization: Bearer ${ZURG_GH_CREDS}" | jq --raw-output '.tag_name // empty')
printf "%s" "${version}"
