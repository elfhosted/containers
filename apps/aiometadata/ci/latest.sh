...
if [[ "${channel}" == "dev" ]]; then
    ...
    version=$(curl -sX GET https://api.github.com/repos/cedya77/aiometadata/tags --header "Authorization: Bearer ${TOKEN}" | jq -r jq -r '.[].name | select((test("beta") | not) and (test("^v5") | not))' | head -n 1)
